...
class dom_tag(object):
    ...
    is_pretty = True   # Text inside the tag should be left as-is (ex. <pre>)
                       # otherwise, text will be escaped() and whitespace may
                       # be modified
    ...
    def __init__(self, *args, **kwargs):
        """
        Creates a new tag. Child tags should be passed as aruguments and
        attributes should be passed as keyword arguments.

        There is a non-rendering attribute which controls how the tag renders:

        * `__inline` - Boolean value. If True renders all children tags on the
                                      same line.
        """
    ...
    def set_attribute(self, key, value):
        ...
        if isinstance(key, int):
            ...
            raise TypeError('Only integer and string types are valid for '
                            'assigning child tags and attributes, '
                            'respectively.')
        ...
        for child in self.children:
            if (isinstance(tag, basestring) and type(child).__name__ == tag) or \
                ...
                if all(child.attributes.get(attribute) == value
                    ...
                    # If the child is of correct type and has all attributes
                    # and values in kwargs add as a result
                    results.append(child)
            if isinstance(child, dom_tag):
                # If the child is a dom_tag extend the search down through
                # its children
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, int):
            ...
            raise TypeError('Only integer and string types are valid for '
                            'accessing child tags and attributes, '
                            'respectively.')
    ...
    def _render(self, rendered, indent=1, inline=False):
        """
        Returns a well-formatted string representation of the tag and
        renderings of all its child tags.
        """
    ...
    def clean_attribute(attribute):
        ...
        underscores. Python also does not support colons in keywords so
        underscores mid-attribute are replaced with colons.
        """
        # shorthand
        attribute = {
            'cls': 'class',
    ...
    def clean_pair(cls, attribute, value):
        """
        This will call `clean_attribute` on the attribute and also allows for
        the creation of boolean attributes.
