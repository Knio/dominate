...
def parse(data, start=0, debug=False, allow_invalid=False, allow_invalid_attributes=False, allow_invalid_markup=False):
  '''
  This method will take a string of XHTML and attempt to parse it into a valid tag tree.
  ...
  * `allow_invalid_attributes` - Boolean value. If True the `__invalid` special keyword will
  ...
  This method will return a the root tag if there is only one or a list of all the adjacent
  top-level tags.
  '''
  if allow_invalid:
